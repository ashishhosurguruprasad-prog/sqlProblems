...
if exists (select name from sys.tables where name='hackers')
drop table hackers
...
select 
      hacker_id,
      count(challenge_id) as total_count
from challenges
group by hacker_id
-- having no_of_challenges= (select max(count(challenge_id)) from challenges)
order by count(challenge_id) desc,hacker_id asc
),
cte2 as (
select
      *,
      max(total_count) over() as max_total,
      count(*) over(partition by total_count) as frequency
from cte1
)
...
where total_count=max_total or frequency=1
